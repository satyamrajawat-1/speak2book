...
from book import (
    ...
    handle_captcha_and_continue,
    handle_payment_selection
)
from login import(
    handle_aadhaar_popup,
    login_irctc
)

from cancel import (
    navigate_to_booked_ticket_history,
)
...
try:
    ...
    weather_book_ticket = input("You Want To Book a Ticket or cancel a Ticket:- ")
    if (weather_book_ticket == "book"):
        ...
        select_class_and_book(
            driver,
            train_number="12951",  #12951  12401
        ...
        #Below the cancel ticket code 
        print("Hello")
        navigate_to_booked_ticket_history(driver)
        time.sleep(30)
        #Hello We Are Here
finally:
        if driver:
            if KEEP_BROWSER_OPEN:
                print("KEEP_BROWSER_OPEN is True — leaving browser open for manual review.")
            else:
                driver.quit()
